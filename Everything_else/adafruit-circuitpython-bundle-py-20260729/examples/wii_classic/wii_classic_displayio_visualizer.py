...
while True:
    ...
    if last_l_x != mapped_l_x:
        analog_text[0].text = f"L_JOY_X: {mapped_l_x}"
        last_l_x = mapped_l_x
    if last_l_y != mapped_l_y:
        analog_text[1].text = f"L_JOY_Y: {mapped_l_y}"
        last_l_y = mapped_l_y
    if last_r_x != mapped_r_x:
        analog_text[2].text = f"R_JOY_X: {mapped_r_x}"
        last_r_x = mapped_r_x
    if last_r_y != mapped_r_y:
        analog_text[3].text = f"R_JOY_Y: {mapped_r_y}"
        last_r_y = mapped_r_y
    if last_r_press != right_pressure:
        analog_text[4].text = f"R_PRESSURE: {right_pressure}"
        last_r_press = right_pressure
    if last_l_press != left_pressure:
        analog_text[5].text = f"L_PRESSURE: {left_pressure}"
